...
//  здесь еще один бессмысленный комментарий для GIT

//LiquidCrystal_I2C lcd(0x27, 16, 2);   // дисплей подключен по i2c адресу 0x27, 16 колонок, 2 строки
LiquidCrystal_I2C lcd(PCF8574_ADDR_A21_A11_A01, 4, 5, 6, 16, 11, 12, 13, 14, POSITIVE);
   // дисплей подключен по i2c адресу 0x27, 16 колонок, 2 строки
...
void fmtNum(uint8_t num, char * buf) {
  uint8_t factor;
  factor=num / 10;
  buf[0] = '0' + factor;
  factor=num % 10;
...
static void doSomeWork()
{
  ...
  lcd.clear();
  lcd.print("lt=");
  lcd.print(fix.location.latF());
  
  lcd.print(" ln=");
//  lcd.println(gps.statistics.ok);
  lcd.print(fix.location.lonF());
  lcd.setCursor(0, 1);
  lcd.print ("sat=");
  lcd.print(gps.sat_count);
  if (fix.valid.time) {
    fmtNum(fix.dateTime.hours, &dateBuf[1]);
    fmtNum(fix.dateTime.minutes, &dateBuf[4]);
    fmtNum(fix.dateTime.seconds, &dateBuf[7]);
  };
  lcd.print(dateBuf);
...
void clearLine(int line = 0) {  //  очищает строчку на дисплее
  lcd.setCursor(0, line); //  начиная с нулевой позиции в указанной строке
  lcd.print(F("                ")); // просто напечтаем строку пробелов
}


void setup()
{
   while (lcd.begin(16, 2, LCD_5x8DOTS) != 1) //colums - 16, rows - 2, pixels - 5x8
